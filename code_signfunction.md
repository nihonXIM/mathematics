## Sine Function Algorithm
### Taylor's theorem

Algorithm to find the sin function:
Input the angle in degrees.
Convert the angle from degrees to radians using the formula radians = degrees * pi / 180.
Set a variable sum to 0.
Set a variable term to the angle in radians.
Set a variable sign to 1.
For i = 1 to n (where n is the number of terms to be used):
a. Add the term to the sum using the formula sum = sum + sign * term.
b. Multiply the term by -1 * angle^2 / (2 * i * (2 * i - 1)).
c. Change the sign of the term to its opposite.
